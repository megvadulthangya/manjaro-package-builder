...
class DatabaseManager:
    ...
    # Self-healing database methods extracted from monolith
    
    def discover_missing_packages(self) -> List[str]:
        """
    ...
    def download_missing_packages(self, missing_packages: List[str]) -> int:
        """
    ...
    def _get_db_package_list(self) -> Set[str]:
        ...
        """
        db_file = self._staging_dir / f"{self.repo_name}.db.tar.gz"
    ...
    def get_recovery_stats(self) -> Dict[str, Any]:
        ...
        return {
            'recovered_packages': len(self._recovered_packages),
            'missing_from_db': len(self._missing_from_db),
            'recovered_list': self._recovered_packages[:10],  # First 10 only
            'missing_list': self._missing_from_db[:10]  # First 10 only
        }